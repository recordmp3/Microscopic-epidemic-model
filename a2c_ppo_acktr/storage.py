...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs_size, action_size,
                 recurrent_hidden_state_size = 0, device = None):
        self.obs = torch.zeros(num_steps + 1, num_processes, obs_size)
        self.recurrent_hidden_states = torch.zeros(num_steps + 1, num_processes, recurrent_hidden_state_size)
        # print("Recurrent =", self.recurrent_hidden_states.shape)
    ...
    def copy(self, r_l):
        for i in range(len(r_l)):
            self.obs[:,i,:] = r_l[i].obs[:,0,:]
            self.recurrent_hidden_states[:,i,:] = r_l[i].recurrent_hidden_states[:,0,:]
            self.rewards[:,i,:] = r_l[i].rewards[:,0,:]
            self.value_preds[:,i,:] = r_l[i].value_preds[:,0,:]
            self.returns[:,i,:] = r_l[i].returns[:,0,:]
            self.action_log_probs[:,i,:] = r_l[i].action_log_probs[:,0,:]
            self.actions[:,i,:] = r_l[i].actions[:,0,:]
            self.masks[:,i,:] = r_l[i].masks[:,0,:]
            self.bad_masks[:,i,:] = r_l[i].bad_masks[:,0,:]
        ...
        if use_proper_time_limits:
            if use_gae:
                ...
                for step in reversed(range(self.rewards.size(0))):
                    ...
                    gae = gae * self.bad_masks[step + 1] # 11.5 when bad_masks is False, PPO will not train on this sample
                    self.returns[step] = gae + self.value_preds[step] # 11.5 gae = value_preds for bad_transition
            else:
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        for i in range(num_mini_batch):
            ...
            for j in range(num_processes):
                ...
                pos = pos * length
                #print('ind and pos', ind, pos)
                obs_batch.append(self.obs[pos:pos + length, ind])
                #print(self.obs[pos:pos + length, ind].shape)
